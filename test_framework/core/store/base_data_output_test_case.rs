...
pub trait BaseDataOutputTestCase {
    ...
    R: Rng + ?Sized,
  {
    ...
    os.close()?;
    drop(os);
    assert_eq!(self.get_bytes(instance), buffer);
    Ok(())
  }
}
...
impl DataInputAction {
  pub fn verify<DI>(&self, src: &mut DI) -> Result<()>
  where
    DI: DataInput,
  {
    match self {
      ...
      DataInputAction::ReadInt(value) => {
        assert_eq!(src.read_int()?, *value, "readInt()");
      },
      DataInputAction::ReadLong(value) => {
        assert_eq!(src.read_long()?, *value, "readLong()");
      },
      DataInputAction::ReadShort(value) => {
        assert_eq!(src.read_short()?, *value, "readShort()");
      },
      DataInputAction::ReadVInt(value) => {
        assert_eq!(src.read_vint()?, *value, "readVInt()");
      },
      DataInputAction::ReadZInt(value) => {
        assert_eq!(src.read_zint()?, *value, "readZInt()");
      },
      DataInputAction::ReadVLong(value) => {
        assert_eq!(src.read_vlong()?, *value, "readVLong()");
      },
      DataInputAction::ReadZLong(value) => {
        assert_eq!(src.read_zlong()?, *value, "readZLong()");
      },
      DataInputAction::ReadString(value) => {
        assert_eq!(src.read_string()?, value.as_str(), "readString()");
      },
    }
    Ok(())
  }
  ...
  for _i in 0..max_add_calls {
    let action = match rnd.random_range(0..GENERATOR_COUNT) {
      ...
      2 => {
        ...
        let bytes: Vec<u8> = (0..len).map(|_| rnd.random()).collect();
        let bytes_len = bytes.len();
        let off = rnd.random_range(0..=bytes_len);
        let length = rnd.random_range(0..=bytes_len - off);
        dst.write_bytes_range(&bytes, off, length)?;
        DataInputAction::ReadBytesRange { bytes, off, length }
      },
      //3 writeInt / readInt
      3 => {
      ...
      6 => {
        let value: i32 = rnd.random();
        dst.write_vint(value)?;
        DataInputAction::ReadVInt(value)
      },
      //7 writeZInt / readZInt
      7 => {
        let value: i32 = rnd.random();
        dst.write_zint(value)?;
        DataInputAction::ReadZInt(value)
      },
      //8 writeVLong / readVLong
      8 => {
        let mut value: i64 = rnd.random();
        value &= (-1i64 as u64 >> 1) as i64;
        dst.write_vlong(value)?;
        DataInputAction::ReadVLong(value)
      },
      //9 writeZLong / readZLong
      9 => {
        let value: i64 = rnd.random();
        dst.write_zlong(value)?;
        DataInputAction::ReadZLong(value)
      },
      //10  writeString / readString
      10 => {
        let length = if rnd.random_range(0..50) == 0 {
          // Occasionally a large blob
          rnd.random_range(2048..=4096)
        } else {
