...
pub struct CrankySegmentInfoFormat<SIF> {
  delegate: SIF,
  random: Arc<Mutex<StdRng>>,
}

impl<SIF> CrankySegmentInfoFormat<SIF> {
  pub fn new(delegate: SIF, random: Arc<Mutex<StdRng>>) -> Self {
    Self { delegate, random }
  }
}

impl<SIF> SegmentInfoFormat for CrankySegmentInfoFormat<SIF>
where
  SIF: SegmentInfoFormat,
{
  fn read<D>(
    &self,
    directory: Arc<D>,
