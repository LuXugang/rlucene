  ...
  BaseIndexFileFormatTestCase<Defaults = BaseTermVectorsFormatTestCaseDefaults>
{
    ...
    R: Rng + ?Sized,
  {
    ...
    for options in self.valid_options() {
      ...
      let mut ids = docs.keys().collect::<Vec<_>>();
      ids.shuffle(random);
...
impl RandomTokenStream {
    ...
    P: FnMut(&mut R) -> Option<BytesRef<Vec<u8>>>,
  {
    ...
    for term in &terms {
      let freq = match freqs.get_mut(term) {
        Some(freq) => freq,
        None => freqs.entry(term.clone()).or_insert(0),
      };
      *freq += 1;
    }
...
impl RandomDocument {
    ...
    R: Rng + ?Sized,
  {
    ...
    for _ in 0..field_count {
      let field_name = loop {
        let field_name = &field_names[random.random_range(0..field_names.len())];
        if used_field_names.insert(field_name) {
          break field_name.clone();
        }
      };
...
fn equals(
  o1: Option<&BytesRef<Vec<u8>>>,
  o2: Option<crate::core::index::BytesRefValueEnum<'_>>,
) -> bool {
  match (o1, o2) {
    (None, None) => true,
