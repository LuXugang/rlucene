...
pub trait Scorable {
    ...
    ///
    /// This score is used when the query/term does not appear in the document, and behaves like an IDF (inverse document frequency).
    /// The smoothing score is particularly important when the scorer returns a product of probabilities,
    /// so that the document score does not go to zero when one probability is zero.
...
///
/// # Fields
/// - `child`: The child `Scorable`. (This is typically a direct child and may itself also have children.)
