...
{
    slice: I::Slice,
    jump_table: Option<I::RandomAccessSlice>,
    ...
    I: IndexInput,
{
    pub fn new(
        ...
        let input = &mut *index_input.borrow_mut();
        let block_slice =
            Self::create_block_slice(input, "docs", offset, length, jump_table_entry_count)?;
        let jump_table = Self::create_jump_table(input, offset, length, jump_table_entry_count)?;
    ...
    pub fn from_components(
        ...
        mut jump_table: Option<I::RandomAccessSlice>,
        jump_table_entry_count: i32,
        dense_rank_power: i8,
        cost: i64,
    ) -> Result<Self> {
        ...
        if let Some(jump) = &mut jump_table {
            if jump.length() > 0 {
                jump.pre_fetch(0, 1)?;
            }
        }
        ...
        O: IndexOutput,
    {
        ...
        if cardinality > IndexedDISI::MAX_ARRAY_LENGTH as i32 {
    ...
    fn advance_block(&mut self, target_block: i32) -> Result<()> {
        ...
        if let Some(jump_table) = &mut self.jump_table {
            if block_index >= (self.block >> 16) + 2 {
                ...
                let offset = jump_table.read_int(jump_pos + BitUtil::INT_BYTES as i64)?;
    ...
    fn read_block_header(&mut self) -> Result<()> {
        ...
        if num_values <= IndexedDISI::MAX_ARRAY_LENGTH as i32 {
...
impl MethodBehavior for DenseMethod {
    ...
    fn advance_exact_within_block<I: IndexInput>(
        &self,
        _disi: &mut IndexedDISI<I>,
        _target: i32,
    ) -> Result<bool> {
        unimplemented!("advance_exact_within_block for DenseMethod is not implemented yet");
    }
}
struct All;
