...
fn test_random_finite_strings1() -> Result<()> {
  ...
  let a = Operations::union_list(&string_list)?;
  ...
  assert_finite_strings_recursive(&a, &actual)?;
...
fn test_single_string() -> Result<()> {
  ...
  assert!(accepted.contains(ints_ref.get()));
  Ok(())
}

/// All strings generated by the iterator.
pub(crate) fn get_finite_strings<T>(iterator: &mut T) -> Result<Vec<IntsRef<Vec<i32>>>>
where
...
fn assert_finite_strings_recursive(
  automaton: &Automaton,
  actual: &[IntsRef<Vec<i32>>],
) -> Result<()> {
  ...
  assert!(
    expected.len() == actual_set.len() && expected.iter().all(|term| actual_set.contains(term)),
    "Expected and actual sets differ: expected={expected:?}, actual={actual_set:?}"
  );
  Ok(())
}
