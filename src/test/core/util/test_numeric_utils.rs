 ...
 */
// Migrated from src/core/util/numeric_utils.rs
...
fn test_long_conversion_and_ordering() -> Result<()> {
  ...
  for value in -100_000..100_000 {
    NumericUtils::long_to_sortable_bytes(value, current.bytes.as_mut_slice(), current.offset);
...
fn test_big_int_conversion_and_ordering() -> Result<()> {
  ...
  for value in -100_000..100_000 {
    ...
    NumericUtils::big_int_to_sortable_bytes(&big_int, size, current.bytes.as_mut_slice(), 0)?;
    if let Some(ref prev) = previous {
      assert!(
        *prev < current,
        "Current value's encoded bytes are not larger than previous"
    ...
    let decoded =
      NumericUtils::sortable_bytes_to_big_int(current.bytes.as_slice(), current.offset, size)?;
    assert_eq!(
      decoded, big_int,
      "Forward and backward conversion failed for value: {}",
      big_int
...
fn test_long_special_values() -> Result<()> {
  ...
  for (i, &value) in values.iter().enumerate() {
    ...
    let decoded_value = NumericUtils::sortable_bytes_to_long(encoded[i].bytes.as_slice(), offset);
    assert_eq!(
      decoded_value, value,
      "Forward and backward conversion failed for value: {}",
      value
...
fn test_int_special_values() -> Result<()> {
  ...
  for (i, &value) in values.iter().enumerate() {
    ...
    let decoded_value = NumericUtils::sortable_bytes_to_int(encoded[i].bytes.as_slice(), offset);
    assert_eq!(
      decoded_value, value,
      "Forward and backward conversion failed for value: {}",
      value
...
fn test_big_int_special_values() -> Result<()> {
  ...
  let values: Vec<BigInt> = vec![
    BigInt::from_i32(i32::MIN).expect("should succeed"),
    BigInt::from_i32(i32::MIN + 1).expect("should succeed"),
    BigInt::from_i32(i32::MIN + 2).expect("should succeed"),
    BigInt::from_i32(-64765767).expect("should succeed"),
    BigInt::from_i32(-4000).expect("should succeed"),
    BigInt::from_i32(-3000).expect("should succeed"),
    BigInt::from_i32(-2000).expect("should succeed"),
    BigInt::from_i32(-1000).expect("should succeed"),
    BigInt::from_i32(-1).expect("should succeed"),
    BigInt::from_i32(0).expect("should succeed"),
    BigInt::from_i32(1).expect("should succeed"),
    BigInt::from_i32(10).expect("should succeed"),
    BigInt::from_i32(300).expect("should succeed"),
    BigInt::from_i32(765878989).expect("should succeed"),
    BigInt::from_i32(i32::MAX - 2).expect("should succeed"),
    BigInt::from_i32(i32::MAX - 1).expect("should succeed"),
    BigInt::from_i32(i32::MAX).expect("should succeed"),
  ];
  let mut encoded: Vec<BytesRef<Vec<u8>>> = values
    .iter()
    .map(|_| BytesRef::from_bytes(vec![0u8; BitUtil::INT_BYTES]))
