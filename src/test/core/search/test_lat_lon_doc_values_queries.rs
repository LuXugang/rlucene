...
where
  F: FnOnce(&TestLatLonDocValuesQueries, &mut StdRng) -> Result<()>,
{
  let mut random = random();
  let case = TestLatLonDocValuesQueries;
  f(&case, &mut random)
}
mod base_geo_point_test_case_tests {
  use crate::core::util::error::lucene_error::Result;
  use crate::test::core::geo::base_geo_point_test_case::BaseGeoPointTestCase;
  use crate::test::core::search::test_lat_lon_doc_values_queries::run_case;
...
impl BaseGeoPointTestCase for TestLatLonDocValuesQueries {
  fn quantize_lat(&self, lat: f64) -> f64 {
    GeoEncodingUtils::decode_latitude(GeoEncodingUtils::encode_latitude(lat).expect(""))
  }

  fn quantize_lon(&self, lon: f64) -> f64 {
    GeoEncodingUtils::decode_longitude(GeoEncodingUtils::encode_longitude(lon).expect(""))
  }
  ...
  fn new_rect_query(
    ...
    LatLonDocValuesField::new_slow_box_query(field, min_lat, max_lat, min_lon, max_lon)
  }
  ...
  fn new_distance_query(
    ...
    LatLonDocValuesField::new_slow_distance_query(field, center_lat, center_lon, radius_meters)
  }

  fn new_polygon_query(&self, field: &str, polygons: Vec<Polygon>) -> Result<Query> {
    LatLonDocValuesField::new_slow_polygon_query(field, polygons)
  }

  fn new_geometry_query(&self, field: &str, geometries: Vec<LatLonGeometryEnum>) -> Result<Query> {
    LatLonDocValuesField::new_slow_geometry_query(field, QueryRelation::Intersects, geometries)
  }
}
