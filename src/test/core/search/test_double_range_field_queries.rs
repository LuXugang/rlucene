...
impl TestDoubleRangeFieldQueries {
    ...
    R: Rng + ?Sized,
  {
    ...
    let mut document = Document::new();
    document.add(DoubleRange::new(FIELD_NAME, [-20.0, -20.0], [30.0, 30.1])?);
    writer.add_document(document)?;

    // intersects (crosses)
    let mut document = Document::new();
    document.add(DoubleRange::new(FIELD_NAME, [-11.1, -11.2], [1.23, 11.5])?);
    writer.add_document(document)?;
    ...
    let mut document = Document::new();
    document.add(DoubleRange::new(FIELD_NAME, [-11.0, -15.0], [15.0, 20.0])?);
    writer.add_document(document)?;
...
impl BaseRangeFieldQueryTestCase for TestDoubleRangeFieldQueries {
    ...
    R: Rng + ?Sized,
  {
    ...
    for d in 0..dimensions {
      ...
      min[d] = CoreHelper::min_f64(min_v, max_v);
      max[d] = CoreHelper::max_f64(min_v, max_v);
    }
