...
struct TestPerFieldPostingsFormat {
  codec: RandomCodec,
}
...
static POSTINGS_TESTER: LazyLock<Mutex<RandomPostingsTester>> = LazyLock::new(|| {
  ...
  Mutex::new(
    RandomPostingsTester::new(&mut random)
      .expect("failed to initialize TestPerFieldPostingsFormat"),
  )
});

impl TestPerFieldPostingsFormat {
...
impl BaseIndexFileFormatTestCase for TestPerFieldPostingsFormat {
...
impl BasePostingsFormatTestCase for TestPerFieldPostingsFormat {
...
where
  F: FnOnce(&TestPerFieldPostingsFormat, &mut StdRng) -> Result<()>,
{
  let mut random = random();
  let case = TestPerFieldPostingsFormat::new(&mut random);
