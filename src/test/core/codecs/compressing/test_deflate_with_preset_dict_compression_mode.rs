...
#[allow(dead_code)]
pub struct TestDeflateWithPresetDictCompressionMode;

fn run_case<F>(f: F) -> Result<()>
where
  F: FnOnce(&TestDeflateWithPresetDictCompressionMode, &mut StdRng) -> Result<()>,
{
  let mut random = random();
  let case = TestDeflateWithPresetDictCompressionMode;
  f(&case, &mut random)
}

impl AbstractTestCompressionMode for TestDeflateWithPresetDictCompressionMode {
