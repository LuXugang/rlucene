...
struct TestAssertingNormsFormat {
  codec: AssertingCodec,
}

impl TestAssertingNormsFormat {
  fn new() -> Self {
    Self {
...
impl BaseNormsFormatTestCase for TestAssertingNormsFormat {}

fn run_case<F>(f: F) -> Result<()>
where
  F: FnOnce(&TestAssertingNormsFormat, &mut StdRng) -> Result<()>,
{
  let mut random = random();
  let case = TestAssertingNormsFormat::new();
...
mod base_norms_format_test_case_tests {
  use crate::core::util::error::lucene_error::Result;
  use crate::test::core::codecs::asserting::test_asserting_norms_format::run_case;
  use crate::test_framework::core::index::base_norms_format_test_case::BaseNormsFormatTestCase;
