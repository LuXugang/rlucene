...
impl BaseNormsFormatTestCase for TestLucene90NormsFormatMergeInstance {}

fn run_case<F>(f: F) -> Result<()>
where
  F: FnOnce(&TestLucene90NormsFormatMergeInstance, &mut StdRng) -> Result<()>,
{
  let mut random = random();
  let case = TestLucene90NormsFormatMergeInstance;
  f(&case, &mut random)
}
