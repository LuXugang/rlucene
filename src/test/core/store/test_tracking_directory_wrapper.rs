 ...
 */
use crate::test_framework::core::util::lucene_test_case::{new_io_context, random};
...
mod base_directory_test_case_tests {
  ...
  use crate::core::util::error::lucene_error::Result;
  use crate::test::core::store::test_tracking_directory_wrapper::run_case;
  use crate::test_framework::core::store::base_directory_test_case::BaseDirectoryTestCase;
...
where
  F: FnOnce(&TestTrackingDirectoryWrapper, &mut rand::prelude::StdRng) -> Result<()>,
{
  let mut random = random();
  let case = TestTrackingDirectoryWrapper;
  f(&case, &mut random)
}
