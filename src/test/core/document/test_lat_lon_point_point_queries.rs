...
struct TestLatLonPointPointQueries;

impl BaseLatLonPointTestCaseHook for TestLatLonPointPointQueries {
  type Shape = Point;
  ...
  fn create_indexable_fields(&self, _name: &str, point: &Point) -> Result<Vec<Fields>> {
    Ok(vec![
      LatLonPoint::new(FIELD_NAME, point.get_lat(), point.get_lon())?.into(),
    ])
  }
}
...
where
  F: FnOnce(
    &BaseLatLonPointTestCaseOwner<TestLatLonPointPointQueries>,
    &mut rand::rngs::StdRng,
  ) -> Result<()>,
{
  let mut random = random();
  let case = BaseLatLonPointTestCaseOwner::new(TestLatLonPointPointQueries);
  f(&case, &mut random)
}

mod lat_lon_point_point_queries_tests {
  use super::*;
