...
use crate::test::core::document::test_lat_lon_doc_values_point_point_queries::PointValidator;
...
struct TestLatLonDocValuesMultiPointPointQueries;

impl BaseLatLonDocValueTestCaseHook for TestLatLonDocValuesMultiPointPointQueries {
  ...
  fn create_indexable_fields(&self, _name: &str, points: &Self::Shape) -> Result<Vec<Fields>> {
    points
      .iter()
      .map(|point| {
        Ok(LatLonDocValuesField::new(FIELD_NAME, point.get_lat(), point.get_lon())?.into())
      })
      .collect()
  }
...
where
  F: FnOnce(
    &BaseLatLonDocValueTestCaseOwner<TestLatLonDocValuesMultiPointPointQueries>,
    &mut rand::rngs::StdRng,
  ) -> Result<()>,
{
  let mut random = random();
  let case = BaseLatLonDocValueTestCaseOwner::new(TestLatLonDocValuesMultiPointPointQueries);
  f(&case, &mut random)
}

mod lat_lon_doc_values_multi_point_point_queries_tests {
  use super::*;
