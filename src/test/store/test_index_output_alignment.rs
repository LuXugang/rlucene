...
fn test_alignment_calculation() -> Result<()> {
    ...
    assert_eq!(align_offset(0, BitUtil::LONG_BYTES)?, 0);
    assert_eq!(align_offset(0, BitUtil::INT_BYTES)?, 0);
    assert_eq!(align_offset(0, BitUtil::SHORT_BYTES)?, 0);
    assert_eq!(align_offset(0, 1)?, 0);

    assert_eq!(align_offset(1, BitUtil::LONG_BYTES)?, 8);
    assert_eq!(align_offset(1, BitUtil::INT_BYTES)?, 4);
    assert_eq!(align_offset(1, BitUtil::SHORT_BYTES)?, 2);
    assert_eq!(align_offset(1, 1)?, 1);

    assert_eq!(align_offset(25, BitUtil::LONG_BYTES)?, 32);
    assert_eq!(align_offset(25, BitUtil::INT_BYTES)?, 28);
    assert_eq!(align_offset(25, BitUtil::SHORT_BYTES)?, 26);
    assert_eq!(align_offset(25, 1)?, 25);

    let val = 1 << 48;
    assert_eq!(align_offset(val - 1, BitUtil::LONG_BYTES)?, val);
    assert_eq!(align_offset(val - 1, BitUtil::INT_BYTES)?, val);
    assert_eq!(align_offset(val - 1, BitUtil::SHORT_BYTES)?, val);
    assert_eq!(align_offset(val - 1, 1)?, val - 1);

    assert_eq!(align_offset(i64::MAX as usize, 1)?, i64::MAX as usize);
    Ok(())
}
#[test]
...
fn test_output_alignment() -> Result<()> {
    ...
    for alignment in alignments.iter() {
        run_test_output_alignment(*alignment)?;
    }
    Ok(())
}
pub fn run_test_output_alignment(alignment: usize) -> Result<()> {
