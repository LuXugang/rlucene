...
use crate::test::util::english::English;
...
fn set_up<R: Rng + ?Sized>(random: &mut R) -> Result<(Vec<SortField>, DefaultIndexSearch)> {
    ...
    for i in 0..limit {
        ...
        if sf.get_type() == SortFieldType::Int {
            let mut sf2 =
                SortField::with_reverse(sf.get_field(), SortFieldType::Int, sf.get_reverse())?;
            ...
            let mut sf2 =
                SortField::with_reverse(sf.get_field(), SortFieldType::Long, sf.get_reverse())?;
    ...
    for i in 0..num_docs {
        ...
        fields.push(
            BinaryDocValuesField::new("bytesval", BytesRef::from_string(&bytesval_value)).into(),
        );
...
fn assert_query<IRC: IndexReaderContext>(
    ...
    let mut all = match sort {
        ...
        Some(ref sort) => {
            let all_manager = TopFieldCollectorManager::with_after(
                sort.clone(),
                max_doc,
                None,
                i32::MAX as usize,
    ...
    while page_start < all.total_hits().value() {
        let mut paged = match sort.clone() {
            ...
            Some(sort) => {
                ...
                let paged_manager = TopFieldCollectorManager::with_after(
                    sort,
                    page_size,
                    after,
                    i32::MAX as usize,
