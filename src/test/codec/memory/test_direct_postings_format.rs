...
static POSTINGS_TESTER: LazyLock<Mutex<RandomPostingsTester>> = LazyLock::new(|| {
  ...
  Mutex::new(
    RandomPostingsTester::new(&mut random).expect("failed to initialize TestDirectPostingsFormat"),
  )
});

impl TestDirectPostingsFormat {
  fn new() -> Self {
    Self {
      codec: TestUtil::always_postings_format(DirectPostingsFormat::new()).into(),
    }
  }
}

impl BaseIndexFileFormatTestCase for TestDirectPostingsFormat {
...
impl BasePostingsFormatTestCase for TestDirectPostingsFormat {
...
where
  F: FnOnce(&TestDirectPostingsFormat, &mut StdRng) -> Result<()>,
{
  let mut random = random();
  let case = TestDirectPostingsFormat::new();
  f(&case, &mut random)
}
