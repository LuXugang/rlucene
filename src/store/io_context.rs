...
impl IOContext {
    ...
    ) -> Result<IOContext, DataIOError> {
        let context = context.ok_or(DataIOError::illegal_argument(
            "context must not be None".to_string(),
        ))?;
        let read_advice = read_advice.ok_or(DataIOError::illegal_argument(
    ...
    fn new_with_read_advice(read_advice: ReadAdvice) -> Result<IOContext, DataIOError> {
        Self::new(Some(Context::Default), Some(read_advice), None, None)
    }

    ///  Creates an `IOContext` for flushing.
    pub fn new_with_flush(flush_info: FlushInfo) -> Result<IOContext, DataIOError> {
        Self::new(
            Some(Context::Flush),
    ...
    pub fn new_with_merge(merge_info: MergeInfo) -> Result<IOContext, DataIOError> {
        Self::new(
            Some(Context::Merge),
            Some(ReadAdvice::Sequential),
            Some(merge_info),
    ...
    #[allow(unused)]
    fn with_read_advice(&self, read_advice: ReadAdvice) -> Result<IOContext, DataIOError> {
    ...
    pub fn default_io_context() -> Result<IOContext, DataIOError> {
    ...
    pub fn read_once_io_context() -> Result<IOContext, DataIOError> {
