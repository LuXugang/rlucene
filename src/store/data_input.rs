...
pub trait DataInput: Sized + Clone {
     ...
     */
    fn read_byte(&self) -> Result<u8, DataIOErrorEnum>;
    /**
     * Reads a specified number of bytes into an array at the specified offset.
     */
     ...
     */
    fn read_short(&self) -> Result<i16, DataIOErrorEnum> {
     ...
     */
    fn read_int(&self) -> Result<i32, DataIOErrorEnum> {
        let b1 = self.read_byte()?;
        let b2 = self.read_byte()?;
     ...
     * random access.
     */
    fn read_group_vint(&self, dst: &mut [i64], offset: i32) -> Result<(), DataIOErrorEnum> {
     ...
     */
    fn read_vint(&self) -> Result<i32, DataIOErrorEnum> {
     ...
     * integer.
     */
    fn read_zint(&self) -> Result<i32, DataIOErrorEnum> {
        Ok(BitUtil::zig_zag_decode_i32(self.read_vint()?))
    }

    fn read_long(&self) -> Result<i64, DataIOErrorEnum> {
     ...
     */
    fn read_longs(&self, dst: &mut [i64], offset: i32, len: i32) -> Result<(), DataIOErrorEnum> {
        let mut i = 0;
        while i < len {
     ...
     */
    fn read_ints(&self, dst: &mut [i32], offset: i32, len: i32) -> Result<(), DataIOErrorEnum> {
        let mut i = 0;
        while i < len {
     ...
     *
     */
    fn read_floats(&self, dst: &mut [f32], offset: i32, len: i32) -> Result<(), DataIOErrorEnum> {
        let mut i = 0;
        while i < len {
     ...
     */
    fn read_vlong(&self) -> Result<i64, DataIOErrorEnum> {
     ...
     */
    fn read_zlong(&self) -> Result<i64, DataIOErrorEnum> {
    ...
    fn read_map_of_strings(&self) -> Result<HashMap<String, String>, DataIOErrorEnum> {
    ...
    fn read_set_of_strings(&self) -> Result<HashSet<String>, DataIOErrorEnum> {
        let count = self.read_vint()?;
        if count == 0 {
            Ok(HashSet::new())
        } else if count == 1 {
    ...
    fn skip_bytes(&self, num_bytes: i64) -> Result<(), DataIOErrorEnum>;
}
