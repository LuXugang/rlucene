...
pub struct OrdinalMap {
  ...
  pub(crate) global_ord_deltas: LongValuesEnum2<PackedLongValues, Zeroes>,
  /// globalOrd -> first segment container
  pub(crate) first_segments: LongValuesEnum2<PackedLongValues, Zeroes>,
    ...
    TE: TermsEnum,
  {
    ...
    let (first_segments_lv, global_ord_deltas_lv) =
      if ord_delta_bits_has_value && ord_delta_bits[0] == 0 && first_segment_bits == 0 {
        ...
        ram_bytes_used +=
          packed_first_segments.ram_bytes_used()? + packed_global_ord_deltas.ram_bytes_used()?;
        (
