...
pub trait DocValuesSkipper {
  ...
  fn advance_with_range(&mut self, min_value: i64, max_value: i64) -> Result<()> {
    if self.min_doc_id_with_level(0) == -1 {
      // `advance` has not been called yet
      self.advance(0)?;
    }
    // check if the current interval intersects the provided range
    while self.min_doc_id_with_level(0) != NO_MORE_DOCS
      && (self.min_value_with_level(0) > max_value || self.max_value_with_level(0) < min_value)
    {
      let mut max_doc_id = self.max_doc_id_with_level(0);
      let mut next_level = 1;
      // check if the next levels intersect to skip as many docs as
      // possible
      while next_level < self.num_levels()
        && (self.min_value_with_level(next_level) > max_value
          || self.max_value_with_level(next_level) < min_value)
      {
        max_doc_id = self.max_doc_id_with_level(next_level);
        next_level += 1;
      }
...
macro_rules! either_docvalues_skipper {
    ($vis:vis $name:ident { $( $Variant:ident : $T:ident ),+ $(,)? }) => {
            ...
            $( $T: DocValuesSkipper ),+
        {
            ...
            fn min_value(&self) -> i64 {
                match self { $( Self::$Variant(inner) => inner.min_value(), )+ }
            }


            fn max_value(&self) -> i64 {
                match self { $( Self::$Variant(inner) => inner.max_value(), )+ }
            }


            fn doc_count(&self) -> i32 {
                match self { $( Self::$Variant(inner) => inner.doc_count(), )+ }
            }
        }
    };
}
