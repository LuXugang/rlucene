...
pub struct SortedSetDocValuesTermsEnum<'a, S>
where
...
impl<'a, S> SortedSetDocValuesTermsEnum<'a, S>
where
    S: SortedSetDocValues,
{
    pub fn new(values: &'a mut S) -> SortedSetDocValuesTermsEnum<'a, S> {
...
impl<'a, S> BytesRefIterator for SortedSetDocValuesTermsEnum<'a, S>
where
    S: SortedSetDocValues,
{
    fn next(&mut self) -> Result<Option<Cow<'_, BytesRef<Vec<u8>>>>> {
        ...
        self.scratch.copy_bytes_with_ref(&term);
        Ok(Some(Cow::Borrowed(self.scratch.get_bytes_ref())))
    }
}

impl<'a, S> TermsEnum for SortedSetDocValuesTermsEnum<'a, S>
where
    S: SortedSetDocValues,
{
    ...
    fn seek_ceil(&mut self, text: &BytesRef<Vec<u8>>) -> Result<SeekStatus> {
        ...
        if ord >= 0 {
            ...
            if self.current_ord == self.values.get_value_count()? {
                ...
                self.scratch.copy_bytes_with_ref(&next_term);
