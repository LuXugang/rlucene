...
pub(crate) struct IntersectTermsEnumFrame {
    ...
    last_state: i32,
    meta_data_upto: i32,
    suffixes_reader: ByteArrayDataInput<Vec<u8>>,
    suffix_lengths_reader: ByteArrayDataInput<Vec<u8>>,
...
impl IntersectTermsEnumFrame {
    pub(crate) fn load_next_floor_block<I, P>(
        ...
        frame_idx: usize,
        stack: &mut [IntersectTermsEnumFrame],
    ) -> Result<()>
    where
        I: IndexInput,
    ...
    pub(crate) fn set_state<I, P>(
        &mut self,
        ite: &IntersectTermsEnum<I, P>,
        state: i32,
    ) -> Result<()>
    where
        I: IndexInput,
        P: PostingsReaderBase<TermState = BlockTermStateEnum>,
    {
        ...
        self.transition_count = ite.automaton.get_num_transitions_with_state(state);
        if self.transition_count != 0 {
            ite.automaton.init_transition(state, &mut self.transition);
            ite.automaton.get_next_transition(&mut self.transition);
        } else {
        ...
        P: PostingsReaderBase<TermState = BlockTermStateEnum>,
    {
        ...
        frame.load(ite, Some(code))
    }
    pub(crate) fn load_from_output_accumulator<I, P>(
        &mut self,
        ite: &mut IntersectTermsEnum<I, P>,
        output_accumulator: &mut OutputAccumulator,
        frame_idx: usize,
        stack: &mut [IntersectTermsEnumFrame],
    ) -> Result<()>
    where
    ...
    {
        let frame = &mut stack[frame_idx];
        output_accumulator.prepare_read();
        let code = ite.fr.read_vlong_output(output_accumulator)?;
        output_accumulator.set_floor_data(&mut frame.floor_data_reader);
        self.load(ite, Some(code))
    }
    fn load<I, P>(
        &mut self,
        ite: &mut IntersectTermsEnum<I, P>,
        ...
        P: PostingsReaderBase<TermState = BlockTermStateEnum>,
    {
        if let Some(block_code) = block_code {
            ...
            if (block_code & OUTPUT_FLAG_IS_FLOOR as i64) != 0 {
                ...
                if !ite.run_automation.is_accept(self.state) && self.transition_count != 0 {
                    ...
                    while self.num_follow_floor_blocks != 0
                        && self.next_floor_label <= self.transition.min
                    {
                        let delta: i64 =
                            (self.floor_data_reader.read_vlong()? as u64 >> 1).try_into()?;
                        self.fp = self.fp_orig + delta;
        ...
        self.ent_count = (code as u32 >> 1).try_into()?;
        ...
        if self.suffix_lengths_reader.bytes.len() < num_suffix_bytes {
            let new_len = ArrayUtil::oversize(num_suffix_bytes, 1);
            self.suffix_lengths_reader.bytes = vec![0u8; new_len];
        }
        ...
        compression_alg.read(
            input,
            &mut self.suffix_lengths_reader.bytes,
            num_suffix_bytes.try_into()?,
        )?;

        self.suffixes_reader.reset_meta(0, num_suffix_bytes);
        ...
        if self.suffix_lengths_reader.bytes.len() < num_suffix_len_bytes {
        ...
        if all_equal {
            ...
            self.suffix_lengths_reader.bytes[..num_suffix_len_bytes].fill(b);
        } else {
            input.read_bytes(
                &mut self.suffix_lengths_reader.bytes,
                0,
        ...
        self.suffix_lengths_reader
            .reset_meta(0, num_suffix_len_bytes);

        // stats
        let num_bytes = input.read_vint()? as usize;
        if self.stats_reader.bytes.len() < num_bytes {
            let new_len = ArrayUtil::oversize(num_bytes, 1);
            self.stats_reader.bytes = vec![0u8; new_len];
        }
        input.read_bytes(&mut self.stats_reader.bytes, 0, num_bytes as i32)?;

        self.stats_reader.reset_meta(0, num_bytes);
        self.stats_singleton_run_length = 0;
        self.meta_data_upto = 0;
        self.term_state.get_block_term_state_mut().term_block_ord = 0;
        self.next_ent = 0;

        // metadata
        let num_bytes = input.read_vint()? as usize;
        if self.bytes_reader.bytes.len() < num_bytes {
            let new_len = ArrayUtil::oversize(num_bytes, 1);
            self.bytes_reader.bytes = vec![0u8; new_len];
        }
        input.read_bytes(&mut self.bytes_reader.bytes, 0, num_bytes as i32)?;

        self.bytes_reader.reset_meta(0, num_bytes);
    ...
    pub(crate) fn next_leaf(&mut self) -> Result<()> {
        ...
        self.next_ent += 1;
        self.suffix = self.suffix_lengths_reader.read_vint()?;
        self.start_byte_pos = self.suffixes_reader.get_position().try_into()?;
    ...
    pub(crate) fn next_non_leaf(&mut self) -> Result<bool> {
        ...
        self.suffix = (code as u32 >> 1).try_into()?;
