...
impl Lucene99HnswVectorsFormat {
  ...
  pub fn with_graph_para(max_conn: usize, beam_width: usize) -> Result<Self> {
    Self::with_graph_para_with_threads(max_conn, beam_width, DEFAULT_NUM_MERGE_WORKER, None)
  }
  ...
  /// - `merge_exec`: executor used by all vector writers generated by this format to do the merge.
  ///
  /// # Errors
  ///
  ...
  pub fn with_graph_para_with_threads(
    ...
    beam_width: usize,
    num_merge_workers: usize,
    merge_exec: Option<Arc<ThreadPool>>,
  ) -> Result<Self> {
    ...
    if num_merge_workers == 1 && merge_exec.is_some() {
    ...
    Ok(Self {
      ...
      beam_width,
      num_merge_workers,
      merge_exec: merge_exec.map(|executor| Arc::new(TaskExecutor::new(executor))),
