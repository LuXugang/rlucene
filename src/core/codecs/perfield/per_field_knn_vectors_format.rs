...
pub struct FieldsWriter<B, O>
where
  B: PerFieldKnnVectorsFormatBase,
  O: IndexOutput,
{
  base: Arc<B>,
  formats: HashMap<Identity, WriterAndSuffix<<B::Format as KnnVectorsFormat>::KnnVectorsWriter<O>>>,
  suffixes: HashMap<String, i32>,
  field_writers: Vec<(Identity, usize)>,
}

impl<B, O> FieldsWriter<B, O>
where
  B: PerFieldKnnVectorsFormatBase,
  O: IndexOutput,
{
  ...
  fn get_instance<D1, D2>(
    ...
    Identity,
    String,
    &mut <B::Format as KnnVectorsFormat>::KnnVectorsWriter<O>,
  )>
  where
...
impl<B, O> KnnVectorsWriter<O> for FieldsWriter<B, O>
where
  B: PerFieldKnnVectorsFormatBase,
  O: IndexOutput,
{
...
impl<B, O> Accountable for FieldsWriter<B, O>
where
  B: PerFieldKnnVectorsFormatBase,
  O: IndexOutput,
{
...
impl<B, O> Closeable for FieldsWriter<B, O>
where
  B: PerFieldKnnVectorsFormatBase,
  O: IndexOutput,
{
        ...
        if field_info.has_vector_values() {
          ...
          let reader = match formats.entry(segment_suffix) {
            ...
            Entry::Vacant(entry) => {
              ...
              let producer = Arc::new(format.fields_reader(&state, segment_info)?);
  ...
  B: PerFieldKnnVectorsFormatBase,
{
  ...
  type KnnVectorsWriter<O: IndexOutput> = FieldsWriter<B, O>;
