  ...
  I: IndexInput + Clone,
{
  ...
  type Bits<'a, B>
    = OffHeapFloatVectorValueBitsEnum<'a, I::RandomAccessSlice, B>
  where
    B: Bits,
    Self: 'a;
...
pub enum OffHeapFloatVectorValueBitsEnum<'a, R, B>
where
  R: RandomAccessInput,
  B: Bits,
{
...
impl<R, B> HasIdentity for OffHeapFloatVectorValueBitsEnum<'_, R, B>
where
  B: Bits,
  R: RandomAccessInput,
{
...
impl<R, B> Bits for OffHeapFloatVectorValueBitsEnum<'_, R, B>
where
  R: RandomAccessInput,
  B: Bits,
{
  ...
  R2: RandomVectorScorer,
{
  ...
  fn iterator(&self) -> Self::DocIdSetIteratorRef<'_> {
    match self {
      VectorScorerEnum::Dense { iterator, .. } => match iterator {
      ...
      VectorScorerEnum::Sparse { iterator, .. } => match iterator {
        DocIdSetIteratorEnum2::A(iterator) => DocIdSetIteratorEnum2::A(iterator),
        DocIdSetIteratorEnum2::B(iterator) => DocIdSetIteratorEnum2::B(iterator),
      },
    }
