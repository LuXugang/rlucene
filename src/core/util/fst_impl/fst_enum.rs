  ...
  F: FstReader,
{
    ...
    FB: FSTEnumBase<O, F>,
  {
    ...
    if target_index < 0 {
      let result = self.backtrack_to_floor_arc(arc_index, target_label, reader, sub, target)?;
    ...
    FB: FSTEnumBase<O, F>,
  {
    ...
    if target_index < 0 {
      ...
      debug_assert!(arc.label() < target_label);
      debug_assert!(arc.is_last() || self.fst.read_next_arc_label(arc, reader)? > target_label);
      self.push_last(sub)?;
      Ok(None)
    }
  }
