    ...
    F: FstReader,
{
    ...
    pub fn read_arc_by_index(
        ...
        input.set_position(
            (arc.pos_arcs_start() - idx as i64 * arc.bytes_per_arc() as i64) as usize,
        );
        arc.arc_idx = idx;
        arc.flags = input.read_byte()?;
    ...
    pub fn find_target_arc(
        ...
        if flags == ARCS_FOR_DIRECT_ADDRESSING {
            ...
            while low <= high {
                let mid = (low + high) >> 1;
                input.set_position(
                    (arc.pos_arcs_start - (arc.bytes_per_arc * mid + 1) as i64) as usize,
                );
    ...
    pub fn seek_to_next_node(&self, reader: &mut impl BytesReader) -> Result<()> {
        loop {
            ...
            if flag_mod(flags as i32, BIT_STOP_NODE)
                && flag_mod(flags as i32, BIT_TARGET_NEXT as i32)
            {
