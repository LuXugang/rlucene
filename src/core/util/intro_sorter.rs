...
pub trait IntroSorter: Sorter {
  fn sort_range(&mut self, from: usize, to: usize) -> Result<()> {
    check_range(from, to)?;
    let max_depth = 2 * (to - from).checked_ilog2().unwrap_or(0) as usize;
    self.sort_in_intro(from, to, max_depth)?;
    Ok(())
  }
  ...
  fn sort_in_intro(&mut self, mut from: usize, mut to: usize, mut max_depth: usize) -> Result<()> {
    while to - from > INSERTION_SORT_THRESHOLD {
      ...
      let mut left_end = j + 1;
      let mut k = from;
      while k < p {
        self.swap(k, left_end - 1)?;
        k += 1;
        left_end -= 1;
      }
      ...
      // by a loop.
      if left_end - from < to - i {
        self.sort_in_intro(from, left_end, max_depth)?;
        from = i;
      } else {
