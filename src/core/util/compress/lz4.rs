...
impl LZ4 {
  ...
  /// Note: This method expects the data to be read in little-endian byte
  /// order. Ensure that the input data is in little-endian format, or it
  /// may result in incorrect parsing.
  ...
  pub fn decompress(
    ...
    loop {
      ...
      if match_dec < match_len || d_off + fast_len > dest_end {
        let mut start: usize = (d_off - match_dec).try_convert()?;
  ...
  /// most 16kB of memory. `[dictOff:dictOff+dictLen]` will be used as a
  ...
  pub fn compress_with_dictionary(
    ...
    if len > LZ4::LAST_LITERALS + LZ4::MIN_MATCH {
      ...
      CoreHelper::check_from_index_size(dict_off as usize, (dict_len + len) as usize, bytes.len())?;
...
impl HashTable for FastCompressionHashTable {
  fn reset(&mut self, off: i32, len: i32) {
    ...
    if need_new_table {
      ...
      // Avoid calling hashTable.clear(), this makes it costly to compress
      // many short sequences otherwise.
      // Instead, get() checks that references are less than the current
      // offset.
    }
  ...
  fn init_dictionary(&mut self, dict_len: i32, bytes: &[u8]) {
    for i in 0..dict_len {
      let v = LZ4::read_int(bytes, self.base + i);
...
impl HashTable for HighCompressionHashTable {
  fn reset(&mut self, off: i32, len: i32) {
    if self.end - self.base < self.chain_table.len() as i32 {
      // The last call to compress was done on less than 64kB, let's not
      ...
      // The last call to compress was done on a large enough amount of
      // data that it's fine to reset both tables
  ...
  fn init_dictionary(&mut self, dict_len: i32, bytes: &[u8]) {
    ...
    for i in 0..dict_len {
      self.add_hash(self.base + i, bytes);
    }
    self.next += dict_len;
  }
...
impl HashTable for HashTableEnum {
  fn reset(&mut self, off: i32, len: i32) {
    match self {
      HashTableEnum::Fast(table) => table.reset(off, len),
      HashTableEnum::High(table) => table.reset(off, len),
    }
  }

  fn init_dictionary(&mut self, dict_len: i32, bytes: &[u8]) {
    match self {
      HashTableEnum::Fast(table) => table.init_dictionary(dict_len, bytes),
      HashTableEnum::High(table) => table.init_dictionary(dict_len, bytes),
    }
  }
...
mod tests {
  ...
  use crate::core::util::compress::lz4::{FastCompressionHashTable, HighCompressionHashTable, LZ4};
  ...
  trait LZ4TestCase {
      ...
      R: Rng + ?Sized,
    {
      let data: Vec<i8> = vec![
        ...
        72, 14, 85, 3, 72, 14, 50, 64, 0, 46, -1, 0, 0, 0, 29, 3, 85, 8, -113, 0, 68, -97, 3, 0, 2,
        3, -97, 6, 0, 68, -113, 0, 2, 3, -97, 6, 0, 68, -113, 0, 2, 3, -97, 6, 0, 68, -113, 0, 2,
        3, -97, 6, 0, 68, -113, 0, 2, 3, -97, 6, 0, 68, -113, 0, 50, 64, 0, 47, -105, 0, 0, 0, 30,
        3, -97, 6, 0, 68, -113, 0, 2, 3, -97, 6, 0, 68, -113, 0, 2, 3, 85, 8, -113, 0, 68, -97, 3,
        0, 2, 3, 85, 8, -113, 0, 68, -113, 0, 2, 3, -97, 6, 0, 68, -113, 0, 2, 3, -97, 6, 0, 68,
        -113, 0, 2, 3, -97, 6, 0, 68, -113, 0, 2, 3, 85, 8, -113, 0, 68, -97, 3, 0, 2, 3, -97, 6,
        0, 68, -113, 0, 2, 3, -97, 6, 0, 68, -113, 0, 2, 3, -97, 6, 0, 68, -113, 0, 2, 3, -97, 6,
        0, 68, -113, 0, 2, 3, -97, 6, 0, 68, -113, 0, 2, 3, -97, 6, 0, 68, -113, 0, 2, 3, -97, 6,
        0, 68, -113, 0, 2, 3, 85, 8, -113, 0, 68, -113, 0, 2, 3, -97, 6, 0, 68, -113, 0, 2, 3, 85,
        8, -113, 0, 68, -97, 3, 0, 2, 3, -97, 6, 0, 68, -113, 0, 2, 3, -97, 6, 0, 68, -113, 0, 2,
        3, -97, 6, 0, 68, -113, 0, 2, 3, -97, 6, 0, 50, 64, 0, 50, 53, 0, 0, 0, 34, 3, 85, 8, -113,
        0, 68, -97, 3, 0, 2, 3, -97, 6, 0, 68, -113, 0, 2, 3, -97, 6, 0, 68, -113, 0, 2, 3, -97, 6,
        0, 68, -113, 0, 2, 3, -97, 6, 0, 68, -113, 0, 2, 3, -97, 6, 0, 68, -113, 0, 2, 3, 85, 8,
        -113, 0, 68, -113, 0, 2, 3, -97, 6, 0, 68, -113, 0, 2, 3, 85, 8, -113, 0, 68, -97, 3, 0, 2,
        3, 85, 8, -113, 0, 68, -97, 3, 0, 120, 64, 0, 52, -88, 0, 0, 0, 39, 13, 85, 5, 72, 13, 85,
        5, 72, 13, 85, 5, 72, 13, 72, 13, 85, 5, 72, 13, 85, 5, 72, 13, 85, 5, 72, 13, 85, 5, 72,
        13, 85, 5, 72, 13, 72, 13, 85, 5, 72, 13, 85, 5, 72, 13, 72, 13, 72, 13, 85, 5, 72, 13, 85,
        5, 72, 13, 85, 5, 72, 13, 85, 5, 72, 13, 85, 5, 72, 13, 85, 5, 72, 13, 85, 5, 72, 13, 85,
        5, 72, 13, 85, 5, 72, 13, 85, 5, 72, 13, 85, 5, 72, 13, 85, 5, 72, 13, 85, 5, 72, 13, 85,
        5, 72, 13, 72, 13, 72, 13, 72, 13, 85, 5, 72, 13, 85, 5, 72, 13, 72, 13, 85, 5, 72, 13, 85,
