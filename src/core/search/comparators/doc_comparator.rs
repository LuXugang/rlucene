...
impl LeafFieldComparator for DocLeafComparator {
  ...
  fn set_scorer<S>(&mut self, _scorer: &mut S, comparator: &mut Self::FieldComparator) -> Result<()>
  where
    S: Scorable + ?Sized,
  {
    self.update_iterator(comparator)?;
...
impl crate::core::search::doc_id_set_iterator::BitSetIteratorAccess for DocComparatorIterator {}
