  ...
  LR: LeafReader,
{
    ...
    S: Scorable + ?Sized,
  {
    ...
    let cmp =
      self.reverse_mul[0] * self.comparators[0].compare_bottom(doc, scorer, &mut comparators[0])?;
    if cmp != 0 {
      return Ok(cmp);
    }
    ...
    S: Scorable + ?Sized,
  {
    ...
    let mut cmp =
      self.reverse_mul[0] * self.comparators[0].compare_top(doc, scorer, &mut comparators[0])?;
    if cmp != 0 {
      return Ok(cmp);
    }
