...
impl DoubleRangeDocValuesField {
    ...
    P: AsRef<[f64]>,
  {
    ...
    let encoded = double_range::encode_range(min, max)?;
    ...
    P: AsRef<[f64]>,
  {
    ...
    let max = max.as_ref();
    Self::check_args(min, max)?;
    DoubleRangeSlowRangeQuery::new(field.into(), min.to_vec(), max.to_vec(), query_type)
  }
...
impl FieldBase for DoubleRangeDocValuesField {
  fn set_bytes_value(&mut self, value: BytesRef<Vec<u8>>) -> Result<()> {
    self.base.set_bytes_value(value)
  }
}

impl IndexableField for DoubleRangeDocValuesField {
  fn name(&self) -> &str {
    self.base.name()
  }
...
impl Display for DoubleRangeDocValuesField {
