...
static TYPE: LazyLock<FieldType> = LazyLock::new(|| {
  ...
  expect_invariant!(
    ...
    "the built-in sorted doc-values field type is mutable and unfrozen"
  );
  ft.freeze();
  ft
});
...
static INDEXED_TYPE: LazyLock<FieldType> = LazyLock::new(|| {
  let mut ft = expect_invariant!(
    ...
    "the built-in sorted doc-values field type can be copied"
  );
  expect_invariant!(
    ft.set_doc_values_skip_index_type(DocValuesSkipIndexType::Range),
    "the copied sorted doc-values field type is mutable and unfrozen"
  );
  ft.freeze();
  ft
});
