...
/// DocValues field for LongRange. This is a single valued field per document due to wrapping a
/// BinaryDocValuesField.
pub struct LongRangeDocValuesField {
...
impl LongRangeDocValuesField {
    ...
    P: AsRef<[i64]>,
  {
    ...
    let encoded = long_range::encode_range(min, max)?;
    ...
    P: AsRef<[i64]>,
  {
    ...
    let max = max.as_ref();
    Self::check_args(min, max)?;
    LongRangeSlowRangeQuery::new(field.into(), min.to_vec(), max.to_vec(), query_type)
  }
...
impl FieldBase for LongRangeDocValuesField {
  fn set_bytes_value(&mut self, value: BytesRef<Vec<u8>>) -> Result<()> {
    self.base.set_bytes_value(value)
  }
}

impl IndexableField for LongRangeDocValuesField {
  fn name(&self) -> &str {
    self.base.name()
  }
...
impl Display for LongRangeDocValuesField {
