...
pub trait IndexableFieldType {
    ...
    ///
    /// This option is illegal if term vectors are not enabled for the field (`store_term_vectors()` is false).
    ...
    ///
    /// This option is illegal if term vector positions are not enabled for the field (`store_term_vectors()` is false).
    fn store_term_vector_payloads(&self) -> bool;

    /// Returns true if normalization values should be omitted for the field.
    ///
    /// Omitting norms saves memory, but at the expense of scoring quality (length normalization will be
    ...
    /// Returns the DocValuesType: how the field's value will be indexed into docValues.
    fn doc_values_type(&self) -> &DocValuesType;

    /// Returns the DocValuesSkipIndexType, indicating whether a skip index for doc values should be created on this field.
    fn doc_values_skip_index_type(&self) -> &DocValuesSkipIndexType;

    /// Returns the number of point dimensions if positive, indicating the field is indexed as a point.
