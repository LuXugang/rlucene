...
/// [`PostingsEnum`] or [`ImpactsEnum`] for the current term [`postings`](TermsEnum::postings).
///
/// Term enumerations are always ordered by `BytesRef::compare_to`, which is Unicode sort order
/// if the terms are UTF-8 bytes. Each term in the enumeration is greater than the one before it.
///
...
pub trait TermsEnum: BytesRefIterator {
    ...
    ///
    /// Seeking by [`TermState`] should only be used if the state was obtained from the same [`TermsEnum`] instance.
    ///
    /// **NOTE**: Using this method with an incompatible [`TermState`] might leave this [`TermsEnum`]
    /// in an undefined state. On a segment level, [`TermState`] instances are compatible only if
    /// the source and target [`TermsEnum`] operate on the same field. If operating on segment level,
    ...
    /// (the sum of `freq()` for each doc that has this term).
    ///
