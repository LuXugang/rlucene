...
/// Controls the health status of [`DocumentsWriter`](crate::index::documents_writer::DocumentsWriter) sessions. This struct blocks
/// incoming indexing threads if flushing is significantly slower than indexing to ensure the
/// [`DocumentsWriter`](crate::index::documents_writer::DocumentsWriter)’s healthiness. If flushing is significantly slower than indexing, the net
...
/// from indexing once 2× the number of available [`DocumentsWriterPerThread`](crate::index::documents_writer_per_thread::DocumentsWriterPerThread)s in
/// [`DocumentsWriterPerThreadPool`](crate::index::documents_writer_per_thread_pool::DocumentsWriterPerThreadPool) is exceeded. Once flushing catches up and the number of flushing
...
impl DocumentsWriterStallControl {
    ...
    /// Sets the stalled flag to `true` if the number of flushing [`DocumentsWriterPerThread`](crate::index::documents_writer_per_thread::DocumentsWriterPerThread)s
    /// exceeds the number of active [`DocumentsWriterPerThread`](crate::index::documents_writer_per_thread::DocumentsWriterPerThread)s. Otherwise, resets the
