...
///
/// A skipper has a position that can only be advanced via [`advance(target)`](DocValuesSkipper::advance).
/// The next advance position must be greater than [`max_doc_id(0)`](DocValuesSkipper::max_doc_id).
/// A skipper's position, along with a `level`, determines the interval at which the skipper is currently situated.
pub trait DocValuesSkipper {
    /// Advance this skipper so that all levels contain the next document on or after `target`.
    ///
    /// NOTE: The behavior is undefined if `target` is less than or equal to `max_doc_id(0)`.
    ///
    /// NOTE: `min_doc_id(0)` may return a doc ID that is greater than `target` if the target document doesn't have a value.
    ...
    /// to the returned value. There is no guarantee that one document actually has this value.
    fn min_value(&self, level: i32) -> i64;

    /// Return the maximum value of the interval at the given level, inclusive.
    ///
    /// NOTE: It is only guaranteed that values in this interval are less than or equal
    /// to the returned value. There is no guarantee that one document actually has this value.
    ...
    ///
    /// NOTE: It is only guaranteed that values are greater than or equal to the returned value.
    ...
    ///
    /// NOTE: It is only guaranteed that values are less than or equal to the returned value.
    /// There is no guarantee that one document actually has this value.
