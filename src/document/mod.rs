...
impl FieldType {
    ...
    pub fn from_ref<I: IndexableFieldType>(ref_field: &I) -> Result<Self, LuceneError> {
    ...
    fn set_dimensions_all(
        ...
        if dimension_count < 0 {
            panic!("dimensionCount must be >= 0; got {}", dimension_count);
        }
    ...
    fn set_vector_attributes(
        ...
        self.check_if_frozen()?;
        if num_dimensions <= 0 {
            panic!("vector numDimensions must be > 0; got {}", num_dimensions);
        }
