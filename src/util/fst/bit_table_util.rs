...
impl BitTableUtil {
    ...
    ///
    /// # Example
    /// A `bit_index` of 10 refers to the third bit on the right of the second byte.
    ///
    /// # Parameters
    /// - `bit_index`: The zero-based index of the bit. It must be greater than or equal to 0 and
    ///   strictly less than `number of bit-table bytes * Byte::SIZE`.
    /// - `reader`: The [`FST::BytesReader`](BytesReader) used for reading. It must be positioned at the beginning
    ///   of the bit-table.
    pub fn is_bit_set<B: BytesReader>(bit_index: i32, reader: &mut B) -> Result<bool, LuceneError> {
    ...
    /// - `reader`: The [`FST::BytesReader`](BytesReader) used for reading. It must be positioned at the beginning
    ///   of the bit-table.
    pub fn count_bits<B: BytesReader>(
        bit_table_bytes: i32,
        reader: &mut B,
    ...
    ///
    /// In other words, counts how many `1`s there are up to (but excluding) the given `bit_index`.
    ///
    /// # Example
    /// A `bit_index` of 10 refers to the third bit on the right of the second byte.
    ///
    /// # Parameters
    /// - `bit_index`: The zero-based index, exclusive. It must be greater than or equal to 0 and
    ///   less than or equal to `number of bit-table bytes * Byte::SIZE`.
    /// - `reader`: The [`FST::BytesReader`](BytesReader) used for reading. It must be positioned at the beginning
    ///   of the bit-table.
    pub fn count_bits_up_to<B: BytesReader>(
        bit_index: i32,
        reader: &mut B,
    ) -> Result<i32, LuceneError> {
        debug_assert!(bit_index >= 0, "bitIndex={}", bit_index);
    ...
    /// Returns the index of the next set bit following the given zero-based index.
    ///
    /// # Example
    ...
    ///
    /// # Parameters
    /// - `bit_index`: The zero-based index of the bit. It must be greater than or equal to -1 and
    ///   strictly less than `number of bit-table bytes * Byte::SIZE`.
    /// - `bit_table_bytes`: The number of bytes in the bit-table.
    /// - `reader`: The [`FST::BytesReader`](BytesReader) used for reading. It must be positioned at the beginning
    ///   of the bit-table.
    ...
    /// Returns the index of the previous set bit preceding the given zero-based index.
    ///
    /// # Example
    /// Given the bit sequence `100011`:
    ...
    ///
    /// # Returns
    /// The zero-based index of the previous set bit before `bit_index`, or `-1` if none exist.
    pub fn previous_bit_set<B: BytesReader>(
        bit_index: i32,
        reader: &mut B,
    ) -> Result<i32, LuceneError> {
        debug_assert!(bit_index >= 0, "bitIndex={}", bit_index);
    ...
    fn read_byte<B: BytesReader>(reader: &mut B) -> Result<u64, LuceneError> {
        let b = reader.read_byte()?;
        Ok((b as u64) & 0xFF)
    }

    fn read_up_to_8_bytes(
        num_bytes: i32,
    ...
    fn bit_count_8_bytes(reader: &mut impl BytesReader) -> Result<i32, LuceneError> {
...
mod tests {
    ...
    use crate::util::fst::bit_table_util::BitTableUtil;

    #[test]
    fn test_next_bit_set() -> Result<(), LuceneError> {
    ...
    #[test]
    fn test_previous_bit_set() -> Result<(), LuceneError> {
        let mut random = random();
        let num_iterations = at_least(&mut random, 1000);
        ...
        for i in 0..num_iterations {
            ...
            // Verify previous_bit_set with count_bits_up_to for all bit indexes.
