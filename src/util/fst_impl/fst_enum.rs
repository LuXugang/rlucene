...
impl<T, O, F> FSTEnum<T, O, F>
where
    T: OutputsBound,
    O: Outputs<T>,
    F: FstReader,
{
    pub(crate) fn new(mut fst: FST<T, O, F>) -> Result<Self> {
    ...
    fn rewind_prefix(&mut self, sub: &mut impl FSTEnumBase) -> Result<()> {
...

        {
            ...
            while self.upto < current_limit
                && self.upto <= self.target_length as usize + 1
            {
                let cmp = sub.get_current_label()? - sub.get_target_label()?;
                if cmp < 0 {
                    break;
                } else if cmp > 0 {
    ...
    pub(crate) fn do_seek_ceil(
        &mut self,
        sub: &mut impl FSTEnumBase,
    ) -> Result<()> {
        ...
        loop {
            ...
            let target_label = sub.get_target_label()?;

            if arc.bytes_per_arc() != 0 && arc.label() != fst_util::END_LABEL {
                let result = match arc.node_flags() {
    ...
    fn do_seek_ceil_array_continuous(
        ...
        if target_index < 0 {
            ...
            sub.set_current_label(arc.label())?;
            self.incr(sub)?;
    ...
    pub(crate) fn do_seek_floor(
        ...
        loop {
            ...
            if arc.bytes_per_arc() != 0 && arc.label() != fst_util::END_LABEL {
                ...
                let result =
                    self.do_seek_floor_list(upto, target_label, sub)?;
                match result {
                    Some(index) => {
                        upto = index;
                    },
    ...
    fn do_seek_floor_continuous(
        ...
        if target_index < 0 {
            self.arcs[upto] = Some(arc);
            let result = self.backtrack_to_floor_arc(
                arc_index,
                target_label,
                reader,
                sub,
    ...
    fn do_seek_floor_array_direct_addressing(
        ...
        if target_index < 0 {
            ...
            let floor_index =
                BitTable::previous_bit_set(target_index, &arc, reader)?;
            ...
            debug_assert!(
                arc.is_last()
                    || self.fst.read_next_arc_label(&arc, reader)?
                        > target_label
            );
            self.arcs[upto] = Some(arc);
            self.push_last(sub)?;
    ...
    /// rollback to last fork then push.
    fn rollback_to_last_fork_then_push(
        &mut self,
        sub: &mut impl FSTEnumBase,
    ) -> Result<()> {
        if self.upto == 0 {
    ...
    /// Backtracks until it finds a node whose first arc is before our target label. Then on that node,
    /// finds the arc just before the `target_label`.
    ///
    ///
    /// # Returns
    ///
    ...
    fn backtrack_to_floor_arc(
        ...
        loop {
            ...
            target_label = sub.get_target_label()?;
            upto = self.upto;
            arc = self.get_arc_ownership(self.upto);
        }
    }
    /// Finds and reads an arc on the current node whose label is strictly less than the given label.
    /// Skips the first arc, finds the next floor arc; or none if the floor arc is the first arc itself
    ...
    fn find_next_floor_arc_continuous(
        ...
        if arc.num_arcs() > 1 {
            ...
            if target_index >= arc.num_arcs() {
                ...
                self.fst.read_arc_by_continuous(
                    arc,
                    reader,
                    target_index - 1,
                )?;
    ...
    fn do_seek_floor_list(
        ...
        if arc.label() == target_label {
            ...
            loop {
                ...
                if arc.label() < target_label {
                    ...
                    while !arc.is_last()
                        && self
                            .fst
                            .read_next_arc_label(&arc, &mut self.fst_reader)?
                            < target_label
                    {
                        self.fst
                            .read_next_arc(&mut arc, &mut self.fst_reader)?;
                    }
                    self.arcs[upto] = Some(arc);
                    self.push_last(sub)?;
                    return Ok(None);
            ...
            let next_label =
                self.fst.read_next_arc_label(&arc, &mut self.fst_reader)?;
    ...
    pub(crate) fn do_seek_exact(
        ...
        let mut arc = self.get_arc_ownership(upto);
        let mut target_label = sub.get_target_label()?;
        let mut fst_reader = self.fst.get_bytes_reader()?;
        loop {
            let next_arc_upto = self.upto;
            let mut next_arc = self.get_arc_ownership(next_arc_upto);
            let found = self.fst.find_target_arc(
                target_label,
                &mut arc,
                &mut next_arc,
                &mut fst_reader,
            ...
            if found.is_none() {
                ...
                self.arcs[next_arc_upto] = Some(next_arc);
                self.arcs[upto] = Some(arc);
                return Ok(false);
            }
            ...
            self.arcs[upto] = Some(arc);
            sub.set_current_label(target_label)?;
            self.incr(sub)?;
            target_label = sub.get_target_label()?;
            upto = self.upto;
            arc = next_arc;
    ...
    fn incr(&mut self, sub: &mut impl FSTEnumBase) -> Result<()> {
        self.upto += 1;
        sub.grow()?;
    ...
    fn push_first(&mut self, sub: &mut impl FSTEnumBase) -> Result<()> {
        let mut upto = self.upto;
        let mut arc = self.get_arc_ownership(upto);
        loop {
            self.output[self.upto] = self
    ...
    fn push_last(&mut self, sub: &mut impl FSTEnumBase) -> Result<()> {
