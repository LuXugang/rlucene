...
impl Outputs<Rc<BytesRc>> for ByteSequenceOutputs {
    fn common(&self, output1: &Rc<BytesRc>, output2: &Rc<BytesRc>) -> Rc<BytesRc> {
        let mismatch_pos = CoreHelper::miss_match(
            ...
            &output2.bytes.borrow()
                [output2.offset as usize..(output2.offset + output2.length) as usize],
        );
    ...
    fn add(&self, prefix: &Rc<BytesRc>, output: &Rc<BytesRc>) -> Rc<BytesRc> {
        ...
        buf.copy_from(
            &prefix.bytes.borrow()
                [prefix.offset as usize..(prefix.offset + prefix.length) as usize],
            0,
        );
        buf.copy_from(
            &output.bytes.borrow()
                [output.offset as usize..(output.offset + output.length) as usize],
