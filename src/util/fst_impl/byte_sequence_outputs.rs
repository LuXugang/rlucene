...
impl Outputs<Rc<BytesRc>> for ByteSequenceOutputs {
    fn common(&self, output1: &Rc<BytesRc>, output2: &Rc<BytesRc>) -> Rc<BytesRc> {
        let mismatch_pos = CommonUtil::miss_match(
            &output1.bytes.borrow()[output1.offset as usize..(output1.offset + output1.length) as usize],
            &output2.bytes.borrow()[output2.offset as usize..(output2.offset + output2.length) as usize],
        );
    ...
    fn add(&self, prefix: &Rc<BytesRc>, output: &Rc<BytesRc>) -> Rc<BytesRc> {
        ...
        buf.copy_from(
            &prefix.bytes.borrow()[prefix.offset as usize..(prefix.offset + prefix.length) as usize],
            0,
        );
        buf.copy_from(
            &output.bytes.borrow()[output.offset as usize..(output.offset + output.length) as usize],
    ...
    fn write(&self, output: &Rc<BytesRc>, out: &mut impl DataOutput) -> Result<()> {
        ...
        out.write_bytes_range(&output.bytes.borrow(), output.offset, output.length)
    }
    ...
    fn read(&self, input: &mut impl DataInput) -> Result<Rc<BytesRc>> {
        ...
        if len == 0 {
            ...
            Ok(Rc::new(BytesRc::from_vec(Rc::new(RefCell::new(output)), 0, len)))
        }
    }
