...
use crate::util::packed::{
    fastest_format_and_bits, Format, FormatAndBits, PackedInts,
};
...
impl AbstractPagedMutableBase for PagedMutable {
    fn new_mutable(
        ...
        let sub_mutable = PackedInts::get_mutable_impl(
            value_count,
            self.bits_per_value,
            self.format,
        )?;
    ...
    fn new_unfilled_copy(
        ...
        let sub_reader = PagedMutable::with_bits_and_format(
            self.bits_per_value,
            self.format,
        );
