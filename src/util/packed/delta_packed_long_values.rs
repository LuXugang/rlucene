...
use crate::util::packed::packed_long_values::{PackedLongValues, PackedLongValuesBase1, PackedLongValuesBase2, PackedLongValuesBuilder};
...
pub struct DeltaPackedLongValuesBuilder {
    base_builder: PackedLongValuesBuilder,
    mins: Vec<i64>
}
impl DeltaPackedLongValuesBuilder {
    ...
    pub fn build(mut self) -> Result<DeltaPackedLongValues, DataIOError> {
        ...
        let values = self.base_builder.values.split_off(self.base_builder.values_off as usize);
...
impl PackedLongValuesBase2 for DeltaPackedLongValuesBuilder {
    ...
    fn pack(&mut self, values: &mut [i64], num_values: u32, block: usize, acceptable_overhead_ratio: f32) -> Result<(), DataIOError> {
