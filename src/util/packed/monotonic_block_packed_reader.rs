...
const BLOCK_SIZE: u32 = u8::BITS; // #bits in a block
const BLOCK_BITS: u32 = 3; //The #bits representing BLOCK_SIZE
const MOD_MASK: u32 = BLOCK_SIZE - 1; //  x % BLOCK_SIZE
/// Provides random access to a stream written with [`MonotonicBlockPackedWriter`](crate::util::packed::monotonic_block_packed_writer::MonotonicBlockPackedWriter).
///
/// # Internal
    ...
    fn new<I: IndexInput>(
        ...
        for i in 0..num_blocks as usize {
            ...
            if bits_per_value == 0 {
                ...
                let size = min(
                    block_size,
                    (value_count - i as u64 * block_size as u64) as u32,
                );
...
impl LongValues for MonotonicLongValues {
    fn get(&mut self, index: u64) -> Result<i64, DataIOError> {
        ...
        let mut end_bits =
            (major_bit_pos & MOD_MASK as u64) as i64 + self.bpv_minus_block_size as i64;
        if end_bits <= 0 {
            // Single block
...
impl LongValues for MonotonicBlockPackedReader {
    fn get(&mut self, index: u64) -> Result<i64, DataIOError> {
        ...
        let idx = (index & self.block_mask as u64) as usize;
        let expected_value = expected(self.min_values[block], self.averages[block], idx);
        let sub_reader_value = self.sub_readers[block].get(idx as u64)?;
...
impl Display for MonotonicBlockPackedReader {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let avg_bpv = if self.sub_readers.is_empty() {
            ...
            self.sum_bpv / self.sub_readers.len() as u64
        };
        write!(
            f,
